...
namespace EthansGameKit.AStar
{
	public sealed class CommonPathfindingSpace : PathfindingSpace<Vector3>
	{
		public sealed class CommonPathfinder : Pathfinder
		{
			...
			public IReadOnlyDictionary<Vector3, Vector3> FromMap => fromMap;
			...
			protected override bool GetCachedParentNode(Vector3 node, out Vector3 fromNode) => fromMap.TryGetValue(node, out fromNode);
			protected override void CacheParentNode(Vector3 node, Vector3 fromNode) => fromMap[node] = fromNode;
